...
it('runs correctly', function (string $input, $expected) {
    ...
    ['let newClosure = fn(a) { fn() { a } }; let closure = newClosure(99); closure()', [EvalInteger::class, 99]],
    ['let newAdder = fn(a, b) { fn(c) { a + b + c } }; let adder = newAdder(1, 2); adder(8)', [EvalInteger::class, 11]],
    ['let newAdder = fn(a, b) { let c = a + b; fn(d) { c + d } }; let adder = newAdder(1, 2); adder(8)', [EvalInteger::class, 11]],
    ['let newAdderOuter = fn(a, b) { let c = a + b; fn(d) { let e = d + c; fn(f) { e + f } } }; let newAdderInner = newAdderOuter(1, 2); let adder = newAdderInner(3); adder(8)', [EvalInteger::class, 14]],
    ['let a = 1; let newAdderOuter = fn(b) { fn(c) { fn(d) { a + b + c + d } } }; let newAdderInner = newAdderOuter(2); let adder = newAdderInner(3); adder(8)', [EvalInteger::class, 14]],
    ['let newClosure = fn(a, b) { let one = fn() { a }; let two = fn() { b }; fn() { one() + two() } }; let closure = newClosure(9, 90); closure()', [EvalInteger::class, 99]],
    ['let countDown = fn(x) { if (x == 0) { 0 } else { countDown(x - 1) } }; countDown(1)', [EvalInteger::class, 0]],
    ['let countDown = fn(x) { if (x == 0) { 0 } else { countDown(x - 1) } }; let wrapper = fn() { countDown(1) }; wrapper()', [EvalInteger::class, 0]],
    ['let wrapper = fn() { let countDown = fn(x) { if (x == 0) { 0 } else { countDown(x - 1) } }; countDown(1) }; wrapper()', [EvalInteger::class, 0]],
    ['let fibonacci = fn(x) { if (x == 0) { 0 } else { if (x == 1) { 1 } else { fibonacci(x - 1) + fibonacci(x - 2) } } }; fibonacci(15)', [EvalInteger::class, 610]],
]);
